...
import { 
  ...
  Cloud, Zap, Home, Key, Smartphone, Monitor
} from "lucide-react";
...
// --- STARTUP CONFIG ---
const defaultCode = `import React from 'react';\nexport default function App() {\n  return (\n    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center p-8 text-center">\n      <h1 className="text-4xl font-black mb-6 italic uppercase tracking-tighter">Odin Forge V2.11</h1>\n      <p className="text-gray-500 uppercase text-[10px] font-black tracking-[0.4em]">Dynamic API Engine Active</p>\n    </div>\n  );\n}\n`;
...
const MODEL_GROUPS = [
  {
    provider: "OpenAI 🟢",
    models: [
      { id: "openai:gpt-4o", name: "GPT-4o (Elite)", icon: Zap },
      { id: "openai:gpt-4o-mini", name: "GPT-4o Mini", icon: Zap },
      { id: "openai:o1-preview", name: "o1 Preview (Logic)", icon: Sparkles }
    ]
  },
  {
    provider: "Anthropic 🟠",
    models: [
      { id: "anthropic:claude-3-5-sonnet-20240620", name: "Claude 3.5 Sonnet", icon: Zap },
      { id: "anthropic:claude-3-5-opus-latest", name: "Claude 3.5 Opus", icon: Sparkles }
    ]
  },
  {
    provider: "Google 🔵",
    models: [
      { id: "google:gemini-1.5-pro", name: "Gemini 1.5 Pro", icon: Zap },
      { id: "google:gemini-1.5-flash", name: "Gemini 1.5 Flash", icon: Zap }
    ]
  },
  {
    provider: "DeepSeek 🛡️",
    models: [
      { id: "nvidia:deepseek/deepseek-v3", name: "DeepSeek V3 (NIM)", icon: Zap },
      { id: "nvidia:deepseek/deepseek-r1", name: "DeepSeek R1 (NIM)", icon: Sparkles }
    ]
  },
  {
    provider: "Local (Ollama) 🏠",
    models: [
      { id: "ollama:llama3", name: "Llama 3 (Local)", icon: Home },
      { id: "ollama:codegemma", name: "CodeGemma (Local)", icon: Home }
    ]
  },
  {
    provider: "Legacy (NIM) 🏹",
    models: [
      { id: "nvidia:qwen/qwen3-coder-480b-a35b-instruct", name: "Qwen3 Coder 480b", icon: Zap },
      { id: "nvidia:mistralai/mistral-small-24b-instruct-2503", name: "Mistral Small", icon: Zap }
    ]
  }
];
...
export default function Workspace() {
  ...
  const [logs, setLogs] = useState<string[]>(["[SYS] Singularity V2.11 Key Forge Active.", "[SYS] Universal Header Bridge Ready."]);
  ...
  const removeKey = (id: string) => setCustomKeys(customKeys.filter(k => k.id !== id));
  ...
  const handleChatAction = async (id: string, action: "pin" | "archive" | "delete" | "rename") => {
    if (action === "delete") {
      ...
      await supabase.from("conversations").update({ is_pinned: !convo?.is_pinned }).eq("id", id);
    } else if (action === "archive") {
      const convo = conversations.find(c => c.id === id);
      await supabase.from("conversations").update({ is_archived: !convo?.is_archived }).eq("id", id);
    } else if (action === "rename") {
      const newTitle = prompt("Enter new title:");
      if (newTitle) await supabase.from("conversations").update({ title: newTitle }).eq("id", id);
    }
    fetchConversations(); setActiveMenuId(null);
  };
                 ...
                 <div className="p-4 space-y-8">
                    ...
                    <div>
                        ...
                        <div className="space-y-2 mb-4">
                           {customKeys.map(k => (
                             <div key={k.id} className="flex items-center justify-between p-3 bg-white/5 border border-white/5 rounded-xl group">
                        ...
                        <div className="bg-black/40 p-3 rounded-2xl border border-white/5 space-y-3">
                           <input placeholder="Name (e.g. Free NIM)" className="w-full bg-transparent border-none text-[10px] font-bold outline-none placeholder:text-white/10" value={newKey.name} onChange={e=>setNewKey({...newKey, name: e.target.value})} />
                           <input placeholder="API Key" className="w-full bg-transparent border-none text-[10px] font-bold outline-none placeholder:text-white/10" type="password" value={newKey.key} onChange={e=>setNewKey({...newKey, key: e.target.value})} />
                           <select className="w-full bg-transparent border-none text-[10px] font-black uppercase outline-none text-white/40" value={newKey.provider} onChange={e=>setNewKey({...newKey, provider: e.target.value})}>
                              <option value="nvidia">NVIDIA (Free)</option>
                              <option value="openai">OpenAI</option>
                              <option value="anthropic">Anthropic</option>
                              <option value="google">Google</option>
                           </select>
                           <button onClick={handleAddKey} className="w-full py-2 bg-white text-black text-[9px] font-black uppercase rounded-lg">Forge Key</button>
                        </div>
                    </div>
      ...
      <div className={`flex-1 flex-col min-w-0 bg-[#09090b] ${isMobile ? "hidden" : "flex"}`}>
         ...
         <div className="h-6 w-full bg-purple-700 flex items-center justify-between px-3 z-50">
            ...
            <div className="flex items-center gap-4 h-full text-[10px] font-black text-white">
               <span>{customKeys.find(k => k.id === selectedModel)?.name || MODEL_GROUPS.flatMap(g => g.models).find(m => m.id === selectedModel)?.name || selectedModel}</span>
               <div className="bg-white/10 px-2 h-full flex items-center">UTF-8</div>
            </div>
         </div>
      </div>

      {/* D. CHAT COLUMN (MOBILE-FIRST) */}
      <AnimatePresence>
        {isRightSidebarOpen && (
          <motion.div initial={{ width: 0 }} animate={{ width: isMobile ? "100%" : 420 }} exit={{ width: 0 }} className="h-full border-l border-white/5 bg-black/20 flex flex-col shrink-0 relative">
             <div className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-widest opacity-60">{isMobile ? "Odin Assistant" : "Universal Eng."}</span>
                </div>
                <button onClick={() => setIsModelDropdownOpen(!isModelDropdownOpen)} className="bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg border border-white/10 text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2">
                  {customKeys.find(k => k.id === selectedModel)?.name.split(" ")[0] || MODEL_GROUPS.flatMap(g => g.models).find(m => m.id === selectedModel)?.name.split(" ")[0]} <ChevronDown className="w-3 h-3" />
                </button>
             </div>
                ...
                {isModelDropdownOpen && (
                   <motion.div initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-10 }} className="absolute top-14 right-6 w-64 bg-[#141416] border border-white/10 rounded-2xl shadow-3xl p-3 z-[400] overflow-y-auto max-h-[70vh] no-scrollbar">
                           ...
                           <div className="space-y-1">
                              {customKeys.map(k => (
                                 <button key={k.id} onClick={() => { setSelectedModel(k.id); setIsModelDropdownOpen(false); }} className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${selectedModel === k.id ? "bg-purple-500/10 text-purple-400" : "text-white/40 hover:bg-white/5 hover:text-white"}`}>
                                    <div className="flex items-center gap-3"><Key className="w-3.5 h-3.5" /><span className="text-[10px] font-black uppercase truncate max-w-[140px]">{k.name}</span></div>
                                    {selectedModel === k.id && <Check className="w-3 h-3" />}
                                 </button>
                              ))}
                           </div>
                        </div>
                      ...
                      {MODEL_GROUPS.map(group => (
                        <div key={group.provider} className="mb-4 last:mb-0">
                           <div className="text-[8px] font-black uppercase tracking-[0.3em] text-white/20 mb-2 px-2">{group.provider}</div>
                           <div className="space-y-1">
                              {group.models.map(m => (
                                 <button 
                                  key={m.id} onClick={() => { setSelectedModel(m.id); setIsModelDropdownOpen(false); }}
                                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${selectedModel === m.id ? "bg-purple-500/10 text-purple-400" : "text-white/40 hover:bg-white/5 hover:text-white"}`}
                                 >
                                    <div className="flex items-center gap-3"><m.icon className="w-3.5 h-3.5" /><span className="text-[10px] font-black uppercase">{m.name}</span></div>
             ...
             <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-6 lg:space-y-8 no-scrollbar pb-32">
                {messages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-10">
                     <Cloud className="w-12 h-12 mb-4" />
                     <div className="text-[10px] font-black uppercase tracking-[0.4em]">Singularity Bridge Active</div>
                  </div>
                )}
                {messages.map((m, idx) => (
                  <div key={m.id || idx} className={`flex flex-col gap-2 ${m.role === "user" ? "items-end" : "items-start"}`}>
                     <div className={`max-w-[92%] px-5 py-4 rounded-[28px] text-[13.5px] leading-relaxed shadow-sm ${m.role === "user" ? "bg-white/5 text-gray-200" : "bg-transparent text-gray-300 border border-white/5"}`}>
             ...
             <div className="p-4 lg:p-6 pt-0 absolute bottom-0 left-0 w-full bg-gradient-to-t from-[#0b0b0d] via-[#0b0b0d] to-transparent">
                <form onSubmit={handleCustomSubmit} className="bg-white/5 border border-white/10 rounded-[32px] p-2 flex flex-col focus-within:ring-4 focus-within:ring-purple-500/10 transition-all shadow-2xl backdrop-blur-3xl">
                   <textarea 
                    className="w-full bg-transparent border-none focus:ring-0 outline-none resize-none px-4 py-3 text-[14px] min-h-[50px] max-h-40 font-medium placeholder:text-white/5"
                    placeholder={isMobile ? "Ask anything..." : "Enter instruction..."} value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),handleCustomSubmit())}
                   />
                   <div className="flex items-center justify-between px-2 pb-1">
                      <button type="button" onClick={()=>fileInputRef.current?.click()} className="p-2.5 text-white/5 hover:text-white transition-all"><Paperclip className="w-4 h-4"/></button>
                      <button type="submit" disabled={isLoading||(!chatInput.trim()&&!selectedImage)} className={`w-10 h-10 flex items-center justify-center rounded-2xl transition-all ${(!chatInput.trim()&&!selectedImage)? "bg-white/5 text-white/5" : "bg-white text-black shadow-xl"}`}><Send className="w-3.5 h-3.5 rotate-[-90deg] translate-x-px"/></button>
                   </div>
                </form>
             </div>
          </motion.div>
